...
const CreatePortfolios = (props:CreatePortfolioProps) => {
    ...
    return (<Grid container rowSpacing={3} columnSpacing={{ xs: 1, sm: 2, md: 3 }} sx={{
        ...
        <Grid item xs={12}>
            <TextField sx={{width:"100%"}} id="portfolio_name" value={portfolioName} 
        ...
        <Grid item xs={12} sx={{textAlign:"left"}}>
            {aircraftList.map((aircraft:string) => {
                return <Chip sx={{margin:"0px 5px 5px 0px"}} key={aircraft} label={(<>{aircraft}
                <Button id={aircraft} style={{height:"16px",width:"16px",border:"0px",marginTop:"5px"}} onClick={onRemoveHandler}>
                    <CloseIcon sx={{height:"20px",width:"20px",marginTop:"-7px"}}/>
                </Button></>)} />
            })}
        </Grid>
        <Grid item xs={12} sx={{textAlign:"right"}}> 
            <Button sx={{marginRight: "10px"}} variant="contained" onClick={onSubmit}>OK</Button>
